...
import { VerifyUserEmail } from "../../Services/Api/VerifyUserEmail";
...
const EmailOtp = () => {
  ...
  useEffect(() => {
    const getEmailPayload = localStorage.getItem("emailResendCodePayload");
    const getTotalTimeValue = localStorage.getItem("emailTimer");

    if (!getEmailPayload || !getTotalTimeValue) {
      navigate("/my-profile");
    } else {
      const parsedEmailNumber = JSON.parse(getEmailPayload);
      const emailAddress = parsedEmailNumber?.email_address;
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      localStorage.setItem("emailTimer", 0);
    };
  ...
  const handleVerify = async (payload) => {
    ...
    if (Response.success === true) {
      ...
      localStorage.removeItem("emailResendCodePayload");
      localStorage.removeItem("emailTimer");
      navigate("/my-profile");
      setLoading(false);
  ...
  const handleResendCode = async () => {
    setErrorMessage("");
    setLoading(true);
    const Response = await ResetUserEmail(resendPayloadData, () => {
      navigate("/");
    });
    if (!Response) {
